...
contract VRToken {
    ...
    uint public totalSupply = 1_000_000_000e18; // 1 billion VRT
    ...
    constructor(address account, address minter_, uint mintingAllowedAfter_) public {
        require(mintingAllowedAfter_ >= block.timestamp, "VRT::constructor: minting can only begin after deployment");
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "VRT::setMinter: only the minter can change the minter address");
    ...
    function mint(address dst, uint rawAmount) external {
        require(msg.sender == minter, "VRT::mint: only the minter can mint");
        require(block.timestamp >= mintingAllowedAfter, "VRT::mint: minting not allowed yet");
        require(dst != address(0), "VRT::mint: cannot transfer to the zero address");

        // record the mint
        mintingAllowedAfter = SafeMath.add(block.timestamp, minimumTimeBetweenMints);

        // mint the amount
        uint96 amount = safe96(rawAmount, "VRT::mint: amount exceeds 96 bits");
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "VRT::mint: exceeded mint cap");
        totalSupply = safe96(SafeMath.add(totalSupply, amount), "VRT::mint: totalSupply exceeds 96 bits");

        // transfer the amount to the recipient
        balances[dst] = add96(balances[dst], amount, "VRT::mint: transfer amount overflows");
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "VRT::permit: amount exceeds 96 bits");
        }
        ...
        require(signatory != address(0), "VRT::permit: invalid signature");
        require(signatory == owner, "VRT::permit: unauthorized");
        require(now <= deadline, "VRT::permit: signature expired");
